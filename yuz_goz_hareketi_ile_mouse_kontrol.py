#!!!!Çalıştırmadan önce Datasets kısmına BİLGİLENDİRME dosyasındaki 2 veri setini dahil etmelisiniz
import cv2
import dlib
...
# değişkenler
color = (0, 255, 0)
thresholdValueright = 0.25
thresholdValueleft = 0.27
thresholdValueboth = 0.52
counterright = 0
realCounterright = 0
counterleft = 0
realCounterleft = 0
    ...
    for face in faces:
        ...
        if earleft < thresholdValueleft:
            ...
            if counterleft >= 3:
                if counterboth < 3:
                    ...
                    print("bilgilendirme...{}.sol göz kırpma tespit edildi...".format(
                        realCounterleft))
                counterleft = 0
        ...
        if earright < thresholdValueright:
            ...
            if counterright >= 3:
                ...
                if counterboth < 3:
                    ...
                    print("bilgilendirme...{}.sağ göz kırpma tespit edildi...".format(
                        realCounterright))
                counterright = 0
        ...
        if earboth < thresholdValueboth:
            ...
            if counterboth >= 3:
                ...
                realCounterboth += 1
                print("bilgilendirme...{}.iki  göz kırpma tespit edildi...".format(
                    realCounterboth))

            counterboth = 0
        # değerlerin görüntü üzerine yazılması
        cv2.putText(frame, "LeftEyeBlink:{}".format(
            realCounterleft), (25, 50), font, 0.8, color, 2)
        cv2.putText(
            frame, "Left- Eye Aspect Ration:{:.2f}".format(earleft), (250, 50), font, 0.8, color, 2)
        cv2.putText(frame, "RightEyeBlink:{}".format(
            realCounterright), (25, 100), font, 0.8, color, 2)
        cv2.putText(frame, "Right Eye Aspect Ration:{:.2f}".format(
            earright), (250, 100), font, 0.8, color, 2)
        cv2.putText(frame, "BothEyeBlink:{}".format(
            realCounterboth), (25, 20), font, 0.8, color, 2)
        cv2.putText(frame, "both Eye Aspect Ration:{:.2f}".format(
            earboth), (250, 20), font, 0.8, color, 2)
